...
تعمل الواجهة والتسجيل محليًا. نسخة iPhone الخاصة تُفتح من Safari برمز دخول؛ وتستخدم
خدمة Railway نفسها لتحليل التسجيل بعد موافقة منفصلة. لا تحتاج نسخة Safari إلى
Firebase، أما بناء iOS الأصلي فيحتاج إعداد Firebase. النموذج تجريبي ولا يصنّف
طلب الحنان أو الضيق ولا يشخّص المغص أو المرض.

## تثبيت النسخة الخاصة على iPhone من دون TestFlight

1. افتحي رابط Railway الخاص في Safari وأدخلي رمز دخول النسخة الذي يصلك منفصلًا.
2. بعد فتح التطبيق، اختاري **مشاركة → إضافة إلى الشاشة الرئيسية**.
3. افتحي أيقونة Baby Monitor من الشاشة الرئيسية واسمحي للميكروفون أو الموقع عند
   استخدام الميزة المطلوبة. يطلب التطبيق موافقتك مرة أخرى قبل رفع أي تسجيل للتحليل.
4. بيانات الطفل وسجل الرعاية تبقى على الجهاز. يحتفظ الخادم بالصوت في الذاكرة أثناء
   التحليل فقط ولا يكتب التسجيل إلى قاعدة بيانات أو ملف.

هذه نسخة ويب قابلة للإضافة للشاشة الرئيسية، وليست IPA أو TestFlight. لا تحتاج
حساب Apple Developer، لكنها تحتاج أن تبقى خدمة التجربة المجانية متاحة.
...
4. أضف القيم العامة التالية في مستودع GitHub من
   **Settings → Secrets and variables → Actions → Variables**:
   ...
   قبل الاتصال بالمزوّد، ويعلن `/v1/health` حالة `assistant_configured` دون كشف
   المفتاح. أثناء التجربة، إذا لم يوجد المفتاح يظل المساعد متاحًا بوضع إرشادي
   محلي محدود (`assistant_mode=offline`) للأسئلة العامة الآمنة، مع بقاء مزود
   النموذج الخارجي غير مفعّل (`assistant_configured=false`). لا يعني نجاح
   `model_ready` أن مزود المساعد النصي متصل؛ فهما حالتان منفصلتان. لا تغني هذه
   الحماية عن المراجعة الطبية.

   إذا كان المزوّد الخارجي مضبوطًا لكنه أعاد حدًا مؤقتًا أو تعذر الاتصال به،
   يرجع الخادم إلى إرشادات محلية آمنة بدل عرض رسالة فشل؛ يظل هذا الرد عامًا ولا
   يُعد تشخيصًا أو وصفًا لدواء. في صفحة الطوارئ، يطلب التطبيق إذن الموقع فقط بعد موافقة ولي الأمر. بعد ذلك
   يمكن فتح بحث منفصل في Google Maps عن مستشفى أطفال أو عيادة أطفال أو طبيب
   أطفال قريب. هذا بحث خرائط يعتمد على النتائج الخارجية وليس دليلًا طبيًا
   معتمدًا، لذلك يجب التأكد من التخصص وساعات العمل قبل الذهاب.
