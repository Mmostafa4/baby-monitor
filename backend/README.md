...
- GET `/call-demo` and WebSocket `/v1/demo-call/ws`: authenticated, in-memory two-device WebRTC audio prototype for testing the future doctor-call flow. It does not collect payments, persist doctor profiles, record calls, or provide a real medical consultation.
